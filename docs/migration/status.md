...
- Browser-client migration now has a React/Vite foundation covering health, catalog profile/session creation, safe profile-detail views, a session directory, session inspection, reconciliation, start/stop, and state inventory. The source terminal, display, audio, remote-device, and hardware views remain blocked on their corresponding target API contracts.
