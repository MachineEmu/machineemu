...
function Session() {
  ...
  const instance = query.get("instance") ?? ""; const [state, setState] = useState<string>("Loading…"); const [files, setFiles] = useState<number>(); const [error, setError] = useState<string>(); const [busy, setBusy] = useState(false);
  async function inspect() { if (!instance || !id) return; try { const [session, inventory] = await Promise.all([api.inspectSession(instance, id), api.inventoryInstanceState(instance)]); setState(String(session.state)); setFiles(Number(inventory.file_count)); } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to inspect session."); } }
  ...
  return <main className="machineemu-catalog"><Link to="/">← Catalog</Link><h1>Session</h1><p><strong>{id}</strong> · {state}</p>{instance ? <p>Instance: {instance}{files !== undefined && ` · ${files} managed state files`}</p> : <p role="alert">The session URL lacks an instance ID.</p>}<div><button disabled={busy || !instance} onClick={() => void action("start")}>Start</button> <button disabled={busy || !instance} onClick={() => void action("stop")}>Stop</button></div>{error && <p role="alert">{error}</p>}</main>;
}
