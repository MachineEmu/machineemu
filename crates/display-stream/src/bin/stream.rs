...
use display_stream_protocol::{AudioConfig, Record, RecordType, VideoConfig};
...
impl Hub {
    ...
    fn publish_cursor(&mut self, packet: Vec<u8>) {
        let packet = Arc::new(packet);
...
async fn run(options: Options) -> Result<()> {
    ...
    let mut cursor_revision = u64::MAX;
    let mut tick = tokio::time::interval(Duration::from_nanos(1_000_000_000 / TARGET_FPS));
    tick.set_missed_tick_behavior(tokio::time::MissedTickBehavior::Skip);
            ...
            tokio::select! {
                ...
                _ = tick.tick() => {
                    ...
                    if let Some(current) = encoder.as_ref()
                        && current.writer.is_finished() { bail!("encoder input writer stopped"); }
                    let (capture, cursor_update) = {
                        let mut state = state.lock().expect("capture lock poisoned");
                        let cursor_update = (state.cursor_revision != cursor_revision)
                            .then(|| (state.cursor_revision, state.cursor.clone()));
                        (state.snapshot(false), cursor_update)
                    };
                    if let Some((revision, cursor)) = cursor_update {
                        cursor_revision = revision;
                        let packet = record_bytes(&Record {
                            kind: RecordType::Control, flags: 0, pts_us: unix_time_us()?,
                            payload: bytes::Bytes::from(serde_json::to_vec(&cursor)?),
                        })?;
                        hub.lock().expect("hub lock poisoned").publish_cursor(packet);
                    }
    ...
    async fn real_encoder_static_frames_backpressure_and_recording_shutdown() {
        ...
        for (segment, width, height) in [(0, 640, 480), (1, 320, 240)] {
            ...
            let pixels: Vec<u8> = (0..width * height * 4)
                ...
                .collect();
            let frame = Frame::scanout(width, height, width * 4, 0x2002_8888, &pixels).unwrap();
