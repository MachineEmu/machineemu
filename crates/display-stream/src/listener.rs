...
impl Listener {
    async fn scanout(
        ...
        self.state
            ...
            .map_err(|error| {
                eprintln!(
                    "Scanout rejected: {error} ({width}x{height}, stride={stride}, pixman=0x{pixman_format:08x})"
                );
    ...
    async fn update(
        ...
        self.state
            ...
            .map_err(|error| {
                eprintln!(
                    "Update rejected: {error} (x={x}, y={y}, {width}x{height}, stride={stride}, pixman=0x{pixman_format:08x})"
                );
                zbus::fdo::Error::Failed(error.to_string())
            })
    }
    ...
    async fn mouse_set(&self, x: i32, y: i32, on: i32) -> zbus::fdo::Result<()> {
        let mut state = self.state.lock().expect("capture lock poisoned");
        state.cursor.x = x;
        state.cursor.y = y;
        state.cursor.visible = on != 0;
        state.cursor_revision = state.cursor_revision.wrapping_add(1);
        Ok(())
    }
    async fn cursor_define(
        ...
        cursor.width = u32::try_from(width)
            .map_err(|_| zbus::fdo::Error::InvalidArgs("negative cursor width".into()))?;
        cursor.height = u32::try_from(height)
            .map_err(|_| zbus::fdo::Error::InvalidArgs("negative cursor height".into()))?;
        cursor.hot_x = hot_x;
        cursor.hot_y = hot_y;
        cursor.data = data;
        if !cursor.valid() || cursor.width > MAX_CURSOR_DIMENSION {
            return Err(zbus::fdo::Error::InvalidArgs("invalid cursor image".into()));
        }
        state.cursor = cursor;
        state.cursor_revision = state.cursor_revision.wrapping_add(1);
        Ok(())
    }
    async fn cursor_move(&self, x: i32, y: i32) -> zbus::fdo::Result<()> {
        let mut state = self.state.lock().expect("capture lock poisoned");
        state.cursor.x = x;
        state.cursor.y = y;
        state.cursor_revision = state.cursor_revision.wrapping_add(1);
    ...
    async fn cursor_update(
        ...
        if x < 0
            ...
            || width < 0
            || height < 0
            || x + width > i64::from(cursor.width)
            || y + height > i64::from(cursor.height)
            || data.len() != width as usize * height as usize * 4
        {
            return Err(zbus::fdo::Error::InvalidArgs(
                "invalid cursor update".into(),
            ));
        }
        let row_bytes = width as usize * 4;
        for row in 0..height as usize {
            let target = ((y as usize + row) * cursor.width as usize + x as usize) * 4;
            cursor.data[target..target + row_bytes]
...
mod tests {
    ...
    #[tokio::test]
    async fn cursor_callbacks_preserve_shape_position_and_visibility() {
        let state = Arc::new(Mutex::new(CaptureState::new()));
        let listener = Listener {
            state: state.clone(),
        };
        listener
            .cursor_define(2, 1, 1, 0, vec![1, 2, 3, 255, 4, 5, 6, 128])
            .await
            .unwrap();
        listener.mouse_set(12, 8, 1).await.unwrap();
        listener
            .cursor_update(1, 0, 1, 1, vec![7, 8, 9, 255])
            .await
            .unwrap();
        let cursor = state.lock().unwrap().cursor.clone();
        assert_eq!(
            (cursor.x, cursor.y, cursor.hot_x, cursor.visible),
            (12, 8, 1, true)
        );
        assert_eq!(cursor.data, vec![1, 2, 3, 255, 7, 8, 9, 255]);
        assert!(
            listener
                .cursor_update(2, 0, 1, 1, vec![0; 4])
                .await
                .is_err()
        );
        listener.mouse_set(12, 8, 0).await.unwrap();
        assert!(!state.lock().unwrap().cursor.visible);
    }
