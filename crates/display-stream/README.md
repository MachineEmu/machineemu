...
machineemu daemon's web proxy. The production encoder is GStreamer's `vah264enc`.
Wire records and control messages live in the sibling `display-stream-protocol`
crate, shared with `machineemu-viewer`.

QEMU's separate cursor plane is delivered as a type-4 control record with
`type: "cursor"`. It carries position, visibility, hotspot, and a bounded
little-endian ARGB bitmap. The latest cursor is replayed to new viewers, and
the native and browser viewers draw it over the decoded video.
