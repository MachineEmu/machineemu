...
def test_registry_resolves_only_the_pinned_build(tmp_path):
    bundle = tmp_path / "bundles" / "qemu-10.2-unifi"
    ...
    release_set.write_text(json.dumps({
        "schema_version": 1,
        "engines": {"qemu-10.2-unifi": {"manifest": "qemu-10.2-unifi/engine-build.json", "build_digest": digest}},
    }), encoding="utf-8")

    manifest, resolved = EngineRegistry.load(release_set, tmp_path / "bundles").resolve(
        "qemu-10.2-unifi", "aarch64-softmmu"
    )
...
def test_registry_rejects_digest_drift(tmp_path):
    ...
    release_set.write_text(json.dumps({
        "schema_version": 1,
        "engines": {"qemu-10.2-unifi": {"manifest": "manifest.json", "build_digest": "a" * 64}},
    }), encoding="utf-8")
    ...
    with pytest.raises(EngineRegistryError, match="digest"):
        EngineRegistry.load(release_set, tmp_path).resolve("qemu-10.2-unifi", "aarch64-softmmu")
